...
Page({
  ...
  onLoad: async function (options) {
    ...
    if (userInfo == {} || userInfo == '' || userInfo == undefined || userInfo == []) {
      ...
      await wx.cloud.callFunction({
        ...
        console.log(res.result, '获取用户的openid');

        wx.setStorageSync('_openid', res.result.openid)
        that.setData({ _openid: res.result.openid })
      }).catch(err => {
        console.log(err)
      })
      ...
      // 通过openid查询用户数据
      wx.request({
        url: app.globalData.requestURL + '/Users/getby_openid', 
        method: 'GET',
        data: {
        ...
        success(res) {
          console.log(res.data, '通过openid查询用户数据')
        ...
        fail(err) {
          console.log('通过openid查询用户数据ERROE', err)
          Toast.fail('系统错误');
        }
      })
    ...
    console.log(userInfo_final, 'userInfo_final')
    ...
    if (userInfo_final != {} && userInfo_final != '' && userInfo_final != [] && userInfo_final != undefined) {
      ...
      wx.request({
        ...
        success(res) {
          console.log(res.data, '获取/Dishes/getall')
          ...
          if (res.data.length > 0) {
            ...
            that.setData({
             ...
             // 基于时间获取菜单列表
      let { start, end } = that.getWeek(new Date());
      if (that.dishes != []) {
        ...
        wx.request({
          ...
          success(res) {
            console.log(res.data, '获取/Menu/getbytime')
            ...
            requestList.forEach(item => { 
              ...
              if (temp.length) 
                menuList[time].list_w[state].list_c.push(...temp)
            })
            menuList.forEach(w => {
              w.list_w.forEach(c => {
            ...
            // if (res.data.length != 0) {
            //   let dishes = res.data.filter(item => item.isInside == true)
            //   console.log(dishes, '改的dishes')

            //   that.setData({ dishes: dishes })
            // } 
            // else {
            //   Toast.fail('系统错误');
            // }
        ...
        fail(err) {
          console.log('menu  请求不到dishes', err);
          Toast.fail('系统错误');
        }
      })
  ...
  getWeek(date) {
    ...
    let startDay = new Date(now.getTime() - (weekday_now - 1) * 24 * 60 * 60 * 1000);
