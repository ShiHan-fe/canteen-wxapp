 ...
 /**
  ...
  onSubmit: async function (e) {
    ...
    for (let i = 0; i < sendData.length; i++) {
      ...
      console.log(sendData[i].state.time)

      // var DATE=util.formatDate(new Date());
      // console.log(DATE)
      wx.request({
        // url: app.globalData.requestURL + '/Recording/update/numberby_id',
        url: 'https://dingcan.meetingcn.cn/dingcan/Recording/update/numberby_id?_id='+(sendData[i]._id)+'&userId='+(sendData[i].userId)+'&number='+(sendData[i].number),
        method: 'PUT',
        // data: temp,
        header: {
            ...
            if (times == sendData.length) {
              ...
              let sendValue = {
                ...
                date2: {
                  ...
                  // value: that.formatDateStr(new Date())
                  value: that.formatDateStr(sendData[i].state.time)
                },
                //温馨提示，20个以内字符
                thing3: {
